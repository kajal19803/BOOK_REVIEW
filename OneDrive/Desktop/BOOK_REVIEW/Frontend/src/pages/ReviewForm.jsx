...
const ReviewForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${API}/api/reviews`,
        { book: id, userId: userInfo._id, rating, comment },
      ...
      {book && (
        <div className="flex flex-col md:flex-row gap-6 items-start mb-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-yellow-600 font-semibold">⭐ {book.rating?.toFixed(1) || "No rating yet"}</p>
          </div>
        </div>
      )}
            ...
            {reviews.map((rev) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <p className="text-blue-700 font-semibold">{rev.user?.name || "Anonymous"}</p>
                  <span className="text-sm text-yellow-600">⭐ {rev.rating}</span>
                </div>
                <p className="text-black">{rev.comment}</p>
                <p className="text-xs text-gray-500 mt-1">{new Date(rev.createdAt).toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
